...
Concernant les optimisations au niveau WebSocket, il s'agit en réalité pour la plupart d'options déjà testées. Comme par exemple utiliser un parseur customisé lors de l'envoi de données binaires. Socket.IO propose également d'utiliser une autre implémentation WebSocket du côté du serveur appelée \mintinline{javascript}{eiows}. Cependant, ce module n'est pas très utilisé ni maintenu et il n'est pas disponible en TypeScript ni sur Windows. Il n'a donc pas été testé. Socket.IO propose également des modules permettant d'accélérer certaines opérations en utilisant des implémentations natives en C++. Le module \mintinline{javascript}{bufferutil} a donc été installé mais les gains de performance n'étaient pas véritablement visibles. Il a quand même été gardé car il s'agit d'une dépendance optionnelle, qui n'est utilisée que lorsqu'elle est disponible.
